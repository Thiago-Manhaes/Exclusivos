...
********************************************************************************************************************* 
* CLIENTE..: Animale                                                                                                *
* OBJETIVO.: Retirar espaÃ§os antes do nome clifor e bloquear cadastro sem conta contabil					                    *
********************************************************************************************************************* 
...
********************************************************************************************************************* 
*- Programa Base de CriaÃ§Ã£o de Objeto de Entrada                                                                    *
********************************************************************************************************************* 
...
*  mostra_nome_obj = .t. mostra o nome dos objetos no tooltip em tempo de execuÃ§Ã£o para facilitar o desenvolvimento *
********************************************************************************************************************* 
* - Atencao !!!!!!!!!!!														                                        *
...
define class obj_entrada as custom
	*- Nome do metodo/funÃ§Ã£o que os objetos linx vÃ£o chamar.
	...
	procedure metodo_usuario
		...
		******************** Metodo chamado pelos Objetos na ValidaÃ§Ã£o
		*   USR_VALID -> Return .f. NÃ£o deixa o Usuario sair do objeto.
		...
		*=messagebox( 'Metodo '+ UPPER(xmetodo) + ' Nome Objeto ' + upper(xnome_obj) )
			...
			do case
				...
				case upper(xmetodo) == 'USR_SAVE_BEFORE' 
					...
					xalias=alias()
						...
						IF V_PRECOS_LOG_00.data_ativacao = DATE()
							IF MESSAGEBOX('Atualização será executada agora, confirma?',4,'Atualização de Preço')=7
								RETURN .f.
							ENDIF	
						ENDIF	
				...
				*#01# - InclusÃ£o do botÃ£o para importaÃ§Ã£o via CSV
			...
			IF this.Parent.ck_varia_cor.Value=0
				...
				*xRecPai = RECNO()
				...
				IF EMPTY(THISFORMSET.LX_FORM1.CONTAINER1.TX_TABELA.VALUE) 
					F_MSG(["Selecione uma tabela de preço.",64,"Atenção!"])
					RETURN .F.
				ENDIF 
				...
				*thisformset.lx_FORM1.lx_pageframe.page2.olecontrol1.Max = lnLinhas
				...
				FOR C=1 TO lnLinhas
					...
					*SET STEP ON	
					...
					xPreco	 	= SUBSTR(laRegistro(C),xposicao+1, LEN(laRegistro(C))) &&------- PREÇO
				...
				IF !EOF()
					**VALIDA SE O CAMPO PREÃ‡O Ã‰ VAZIO
					SCAN 
					...
					**VALIDA SE O PRODUTO QUE FOI IMPORTADO EXITE NA TABELA PRODUTOS
					SCAN
						f_wait("Verificando se o produto ["+ALLTRIM(CUR_IMPORT.PRODUTO) + "] está cadastrado...") 
						thisformset.LX_gera_cursor_importacao(1,"cur_valida")
						SELECT cur_valida
						GO TOP 
						IF EOF()
							MESSAGEBOX("PRODUTO: [" + ALLTRIM(CUR_IMPORT.PRODUTO) +"] não esta cadastrado. Verifique",64,"Atenção")
							RETURN .F.
						ENDIF 
						SELECT CUR_IMPORT
					ENDSCAN 
					...
					**VALIDA SE O PRODUTO QUE FOI IMPORTADO EXITE NA TABELA PRODUTOS, SE NÃƒO EXISTIR INCLUI O PRODUTO COM VALOR ZERADO.
					nCount = 0
					SCAN
						thisformset.LX_gera_cursor_importacao(3,"cur_valida")
						f_wait("Verificando se o produto ["+ALLTRIM(CUR_IMPORT.PRODUTO) + "] está cadastrado na tabela de preço...") 
						SELECT cur_valida
						GO TOP 
						IF EOF()
							...
							TRY
								...
								nCount = nCount + 1
							CATCH
								MESSAGEBOX("Não foi possível cadastrar o PRODUTO: [" + ALLTRIM(CUR_IMPORT.PRODUTO) +"] na tabela de Preço selecionada." + CHR(10)+CHR(13)+"Verifique",64,"Atenção")
								RETURN .F.	
							ENDTRY
						ENDIF 
						SELECT CUR_IMPORT
					...
					**
					SCAN 
						...
						*WAIT windows 'Processando produto ['+ALLTRIM(CUR_IMPORT.PRODUTO) + ']' NOWAIT 
						...
						ELSE 
							F_MSG(["Nenhum produto encontrado. Verifique se o(s) produto(s) se encontram cadastrado(s).",64,"Atenção!"])
							RETURN .f.
						ENDIF    
						SELECT CUR_IMPORT
					ENDSCAN 
					...
					IF RECCOUNT()>0
						MESSAGEBOX("Processo de importação, realizado com sucesso."+CHR(10)+CHR(13)+"Adicionado " + ALLTRIM(STR(nCount)) + " Produtos na Tabela de preço, pois não existia",64,"Atenção!")
					ELSE
						F_MSG(["Nenhum produto encontrado. Verifique de o(s) produto(s) se encontram cadastrado(s).",64,"Atenção!"])
						RETURN .f.
					ENDIF 
					...
					*GO xRecPai
					this.Enabled= .F. 
				ELSE 
					F_MSG(["Nenhum registro encontrado no arquivo CSV. Verifique!",64, "Atenção!"])
					RETURN .F. 
				ENDIF 
			ELSE &&---------------VARIAÃ‡ÃƒO DE COR---------------------------------
				xImport = .t.
				...
				IF !EOF()
					**VALIDA SE O CAMPO PREÃ‡O Ã‰ VAZIO
					...
					SCAN 
						...
						IF EOF()
							MESSAGEBOX("O Produto: [" + ALLTRIM(CUR_IMPORT_COR.PRODUTO) +"] não esta cadastrado ou sem variação de preço por cor. Verifique!",64,"Atenção!")
							RETURN .F. 
						ENDIF 
						
						IF f_vazio(CUR_IMPORT_COR.COR_PRODUTO)
							MESSAGEBOX("O Produto: ["+ALLTRIM(CUR_IMPORT_COR.PRODUTO)+"] não tem cor informada. Verifique!!",64,"Atenção!")
							RETURN .f.
						ENDIF 
						
						IF f_vazio(CUR_IMPORT_COR.preco1)
							MESSAGEBOX("O Produto: ["+ ALLTRIM(CUR_IMPORT_COR.PRODUTO) +"] não tem preço informado. Verifique!",64,"Atenção!")
							RETURN .f.
						ENDIF 					
						SELECT CUR_IMPORT_COR
					ENDSCAN 
					...
					IF RECCOUNT()>0
						...
						WITH THISFORMSET.lx_FORM1.cmd_importar
							.ENABLED = .F.
						ENDWITH
						F_MSG(["Processo de importação, realizado com sucesso.",64,"Atenção!"])
						THISFORMSET.lx_FORM1.LX_PAGEFRAME1.page2.activate()
						THISFORMSET.Refresh()
					ELSE
						F_MSG(["Nenhum produto encontrado. Verifique de o(s) produto(s) se encontram cadastrado(s).",64,"Atenção!"])
					...
					*GO xRecPai
					this.Enabled= .F. 
				ELSE 
					F_MSG(["Nenhum registro encontrado no arquivo CVS. Verifique!",64, "Atenção!"])
					RETURN .F. 
				ENDIF 
			ENDIF 	
		ENDIF  
