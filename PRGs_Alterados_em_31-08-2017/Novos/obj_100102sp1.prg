			...
			*- metodo do inicio da form
			case UPPER(xmetodo) == 'USR_INIT' 
				** Tiago Carvalho (SS) - 13/10/2016 - Cria o botão para criação de Nota Fiscal em lote importado via excel na tela SS0065
				...
				IF !(TYPE("Thisformset.PP_SS_PERMITE_GERAR_NF_EXCEL") <> "U" AND Thisformset.PP_SS_PERMITE_GERAR_NF_EXCEL)
					** BRUNO SILVA (SS) - 23/01/2017 - adicionado variavel xSSPedidos - (utilizado na geração de notas de royalties)							
					...
					** Tiago Carvalho (SS) - 13/10/2016 - Se tiver gerando nota via excel salvo o numero da nota na tabela importada pelo excel
					TEXT TO lcSql TEXTMERGE noshow
						UPDATE A
								...
								WHERE ORIGEM.MATRIZ_FISCAL = '<<v_faturamento_05.filial>>'
									and A.FILIAL_DESTINO = '<<v_faturamento_05.nome_clifor>>'
									and a.NOME_ARQUIVO = '<<lcStrNomeArquivo>>'
									and A.ID_NOTA = '<<lcIntIdNota>>'
									and A.NATUREZA_SAIDA='<<v_faturamento_05.natureza_saida>>'
			...
			strcaminho = getfile("xls?","Importar Arquivo","Importar",0,"Importar Arquivo")
					...
					try
						...
						IF upper(alltrim(objsheet.cells(1,1).text)) <> "CODIGO_ITEM" OR upper(alltrim(objsheet.cells(1,2).text))<> "QTDE_ITEM" OR upper(alltrim(objsheet.cells(1,3).text))<> "PRECO_UNITARIO"
							MESSAGEBOX("O Layout do Arquivo é Inválido, O Cabeçalho deve Conter as Seguintes Informações:"+CHR(10)+CHR(10)+"Célula A: CODIGO_ITEM"+CHR(10)+"Célula B: QTDE_ITEM"+CHR(10)+"Célula C: PRECO_UNITARIO",0+16,"Layout Inválido")
							return .f.
							*GO to oexception
						ENDIF
...
*!*				IF 	RECCOUNT() > 400
*!*					MESSAGEBOX("Somente é permitido um máximo de 400 itens por nota fiscal, Favor Corrigir !!!",0+64,"Erro ao Importar")
*!*					RETURN .F.
*!*				ENDIF	
		...
		TEXT TO lcSql TEXTMERGE NOSHOW 
			SELECT	A.NATUREZA_SAIDA,
						...
						MENSAGEM_ERRO = CONVERT(VARCHAR(240),
										...
										CASE WHEN  B.INATIVO = 1 THEN 'PROD INATIVO | '+CHAR(10) ELSE '' END +
