	...
	procedure metodo_usuario
		...
		*=messagebox( 'Metodo '+ UPPER(xmetodo) + ' Nome Objeto ' + upper(xnome_obj) )
		do case
			...
			** WRP - Codigo para bloqueio da lei da moda					
				IF upper(xnome_obj)=='TV_NATUREZA_SAIDA'
					
					SELE v_faturamento_05

					xcod_nat_sai = v_faturamento_05.natureza_saida
										
						
						IF (xcod_nat_sai = '120.04')
							
							MESSAGEBOX("Está natureza é exclusiva para lei da moda não pode ser usada nessa tela")
							RETURN .f.

						endif
						*xverifica= xselverifica.validacao
						
						
						*MESSAGEBOX(xverifica)
				ENDIF
			** FIM - WRP - Codigo para bloqueio da lei da moda
			
				
				*** Inicio - MB - Codigo para bloqueio de filiais bloqueadas nas operações de naturezas de saida ***
				SELE v_faturamento_05
				IF upper(xnome_obj)=='TV_NOME_CLIFOR'
					TEXT TO xselverifica2 TEXTMERGE NOSHOW
						SELECT FILIAL FROM GS_NATUREZAS_FILIAIS_BLOQUEADAS WHERE FILIAL='<<V_FATURAMENTO_05.nome_clifor>>' AND NATUREZA='<<v_faturamento_05.natureza_saida>>'
					ENDTEXT
					
					f_select(xselverifica2,"Cur_XVerificaBloqueio")
					IF Cur_XVerificaBloqueio.FILIAL = V_FATURAMENTO_05.nome_clifor
						MESSAGEBOX("Está operação não poderá ser realizada com a natureza de operação " + v_faturamento_05.natureza_saida + CHR(10)+CHR(13) + "Natureza Bloqueada para a Filial "+ V_FATURAMENTO_05.nome_clifor)
						RETURN .f.
					ENDIF
				*****			
				ENDIF
				*** FIM - MB - Codigo para bloqueio de filiais bloqueadas nas operações de naturezas de saida ***
			 	SELE v_faturamento_05
			...
			case UPPER(xmetodo) == 'USR_SAVE_BEFORE' 
				...
				** Tiago carvalho (SS) -06/06/2017 - Não permite faturar se a consolidação do WMS não for finalizada.					
				IF USED("v_faturamento_05_estoque_sai1_mat") AND RECCOUNT("v_faturamento_05_estoque_sai1_mat") > 0
					SELECT v_faturamento_05_estoque_sai1_mat
					GO TOP 
					SCAN
						lcReqMaterial =  ALLTRIM(v_faturamento_05_estoque_sai1_mat.req_material)
						
						TEXT TO lcSQl TEXTMERGE noshow
							select ORDEM_SERVICO 
								from ss_picking_materiais 
								where REQ_MATERIAL ='<<lcReqMaterial>>'
								and ORDEM_SERVICO  in (select ORDEM_SERVICO from ss_picking_materiais where REQ_MATERIAL is null)
						ENDTEXT
						 
						f_select (lcSQl ,"CurConsolidacaoPendente")
						
						IF RECCOUNT("CurConsolidacaoPendente") > 0
							MESSAGEBOX("Existe consolidação pendente no WMS para a saída "+lcReqMaterial +" verifique Antes de Continuar!",0+16,"Consolidação Pendente")
							RETURN .f.
						ENDIF
					ENDSCAN
				ENDIF
			...
			case UPPER(xmetodo) == 'USR_WHEN' AND (upper(xnome_obj)=='TX_REQ_MATERIAL'  OR upper(xnome_obj)=='TX_FILIAL')
			
				*** #1 - LUCAS MIRANDA - CONTROLE SAIDA AVIAMENTOS - 29/09/2016
				TEXT TO xVerPedido TEXTMERGE NOSHOW
									
					SELECT DISTINCT ORDEM_PRODUCAO AS PEDIDO, C.VALOR_PROPRIEDADE AS FILIAL_FABRICA
					...
					AND C.PROPRIEDADE = '00248'
					AND C.VALOR_PROPRIEDADE = 'SIM'
					WHERE REQ_MATERIAL = '<<V_ESTOQUE_SAI_MAT_00.req_material>>'
										
				ENDTEXT
									
				f_select(xVerPedido,"VerPedido",ALIAS())
				SELE VerPedido
 
				If RECCOUNT("VerPedido") > 0 AND v_faturamento_05.ctb_tipo_operacao <> 199 AND Thisformset.pp_anm_valid_saidas_mp_pa = .t.
					MESSAGEBOX("Essa saída não poderá ser escolhida, pois é uma saída de P.A. !"+CHR(13)+"Favor utilizar a natureza Outras Saídas !",0+16)	
					Return .F.
				Endif
				*** #1 - LUCAS MIRANDA - CONTROLE SAIDA AVIAMENTOS - 29/09/2016			
