	...
	procedure metodo_usuario
		...
		*=messagebox( 'Metodo '+ UPPER(xmetodo) + ' Nome Objeto ' + upper(xnome_obj) )
			 ...
			 *Inicio Inclusao Campos Novos de Produtos no Cursor Pai -- v_producao_os_01   
				...
				** Inclusão do campo Lote - Lucas Miranda
				oCur.addbufferfield("PRODUCAO_ORDEM_SERVICO.GS_LOTE_OS", "N(10)",.T., "Lote O.S.", "PRODUCAO_ORDEM_SERVICO.GS_LOTE_OS")
			...
			*Inicio Inclusao Campos Novos de Produtos no Cursor Pai -- v_producao_os_01   
			 ...
			 * Inclui Nova Procedure na Classe da Tela >> Dentro de Activate da Guia Materiais
				...
				f_select( "SELECT '' AS FILIAL UNION ALL "+ ;
						  +"SELECT VALOR_PROPRIEDADE AS FILIAL FROM PROPRIEDADE_VALIDA WHERE PROPRIEDADE = '00576'","CurFilOS")
				thisformset.lx_FORM1.cmb_FILIAL.RowSource="CurFilOS.filial"
				*thisformset.lx_FORM1.cmb_FILIAL.ControlSource="CurFilOS.filial"
					
					*filtro por parâmetro para impressao de OS - silvio luiz 07/12/2017		
...
*!*							
*!*				
*!*					*** Projeto Composição O.S. - Lucas Miranda
*!*					
*!*						if thisformset.p_tool_status $'I'
*!*							
*!*								ReplaceComposicao=''
				...
				case upper(xmetodo) == 'USR_SAVE_BEFORE'	
				  ...
				  *Silvio 05/04/2018 - Guardar valor da propriedade 00046
				  TEXT TO xsel NOSHOW TEXTMERGE
						...
						and ordem_servico='<<v_producao_os_01_tarefas.ordem_servico>>'				
				  ENDTEXT
					...
					IF thisformset.p_tool_status $'IA'
					 	...
					 	IF ALLTRIM(V_PRODUCAO_OS_01_ANTERIOR.DESC_FASE_PRODUCAO) = 'FABRICA'
							...
							IF Cur_CustoFab.CUSTO_TAREFA <> V_PRODUCAO_OS_01_ANTERIOR.CUSTO_EFETIVO+V_PRODUCAO_OS_01_TAREFAS.custo_previsto
								MESSAGEBOX('CUSTOS DE FACÇÃO\FABRICA COM VALORES INCORRETOS :'+CHR(13)+CHR(13)+;
								...
								'CUSTO INFORMADO FACÇÃO:    R$ '+ALLTRIM(STR(V_PRODUCAO_OS_01_TAREFAS.custo_previsto))+CHR(13)+CHR(13)+;
								'CUSTO PEDIDO PRECISA SER IGUAL AO'+CHR(13)+'CUSTO FABRICA+CUSTO FACÇÃO',0+64)
...
*!*									MESSAGEBOX('Propriedades CHECKLIST STATUS pendente. Verifique!',0+48,"Atenção")	 
*!*									RETURN .f.	 
*!*							ENDIF
				 ...
				 *Silvio 05/04/2018 - Guardar valor novo da propriedade 00046 e inclui na tabela de log
				  	...
				  	IF x_val_prop_novo.valor_propriedade<>x_val_prop_antigo.valor_propriedade
				  			...
				  			text to	xinsert_Log noshow textmerge	
								...
								SELECT 
								'<<v_producao_os_01_tarefas.ordem_servico>>','<<x_val_prop_novo.valor_propriedade>>','<<x_val_prop_antigo.valor_propriedade>>',(SELECT GETDATE()),'<<WUSUARIO>>'
							endtext
...
*!*								IF MESSAGEBOX('Deseja pesquisar todas as Ordens de Serviços?',4+32)=7
*!*									RETURN .F.
*!*								ENDIF
*!*							ENDIF		
...
*!*							ENDIF
	...
	** início customização impressao OS - Silvio - 04/12/2017
...
** fim customização impressao OS - Silvio 
...
*!*								Go Top
*!*								
*!*								IF MESSAGEBOX('Deseja Orderna Visualização por prioridade de OS?',+4+32+256)=6
*!*									
*!*									Scan
*!*															
*!*										f_prog_bar("Pequisando Programação da OS: "+ALLTRIM(v_producao_os_01.ORDEM_SERVICO),RECNO(),RECCOUNT())
*!*										
...
*!*									RETURN .f.	
*!*	*!*									MESSAGEBOX("Proibido alterar OS com notas Emitidas,"+CHR(13)+"somente a aba Propriedades ficará disponível"+CHR(13)+"para alteração !!!",0+48)
...
*!*								ENDIF			
							...
							IF ALLTRIM(v_producao_os_01.anm_status_almox) == "1-ENVIADO PARA ALMOX"
								messagebox("Você Não tem Permissão para Alterar a OS com o Status de Enviado Para Almox, Favor Entrar em contato com o Almoxarifado para a liberação",48,"Atenção!!!")
								return .F.
							ENDIF
	...
	* Inclui Nova Procedure na Classe da Tela >> Dentro de Activate da Guia Materiais
	...
	Procedure AnmBuscaSeparaAlmox 
		...
		go top	
		scan	
			*f_select("select convert(varchar(25),case when anm_reserva_almox=1 then 'SEPARA' else 'NÃO SEPARA' end) as anm_reserva_almox " + ; 
	...
	If thisformset.p_tool_status="P"
		...
		If v_producao_os_01.anm_status_almox="1-ENVIADO PARA ALMOX" 
			...
			if f_vazio(curUserFimOs.valor_propriedade)
				messagebox("Você Não tem Permissão para Alterar o Status da OS enquanto não for Finalizada",48,"Atenção!!!")
				thisformset.lx_form1.cmb_status_entrada.enabled=.F.
				retu .f.
				o_toolbar.Botao_refresh.Click()
			else
				if messagebox("Deseja Alterar o Status da Ordem de Serviço?",4+32+256,"Atenção!")=6	
					thisformset.lx_form1.cmb_status_entrada.enabled=.T.
				endif
			endif				
		Else
...
DEFINE CLASS cmb_status_entrada AS lx_combobox
	...
	IF THISFORMSET.P_TOOL_STATUS="P"
		...
		If v_producao_os_01.anm_status_almox <> "1-ENVIADO PARA ALMOX" 
			...
			if messagebox("Deseja Finalizar esta Ordem de Serviço?",4+32+256,"Atenção!")=6	
		...
		Else
			...
			if messagebox("Deseja Alterar o Status desta OS para "+allt(v_producao_os_01.anm_status_almox)+" ?",4+32+256,"Atenção!")=6
				...
				** Lucas Miranda - 16/04/2018 - Bloquear Envio de OS/Pedido com Consumo Pendente a Faturar
				...
				TEXT TO xBloqNFConsP TEXTMERGE NOSHOW					
					...
					AND A.NF_PENDENTE = 1
					AND B.ORDEM_PRODUCAO in (select distinct b.ORDEM_PRODUCAO from PRODUCAO_ORDEM_SERVICO (nolock) a 
												join producao_os_tarefas (nolock) b
				...
				IF !F_Vazio(CurBloqNFConsP.req_material)
					MESSAGEBOX("ATENÇÃO!"+CHR(13)+"Impossível Enviar O.S., Consumo do Pedido com N.F. Pendente !",64)
					replace v_producao_os_01.anm_status_almox with xOldStatusAlmox
					...
					** Reajuste Risco/Piloto - Lucas Miranda - 27/02/2018
					...
					If RECCOUNT("curTbEnviaRiscoP")=0
						...
						if !f_insert(xTbEnviaRiscoP) 
							messagebox("Não foi possível Gravar na Tabela GS_ENVIA_RISCO_PILOTO !!!",48,"Atenção_8!")
						Endif
					Endif
					** Início Risco
					...
					IF RECCOUNT("CurContemFat") > 0
						TEXT TO xUpdateEnviaRisco0 TEXTMERGE NOSHOW
							UPDATE GS_ENVIA_RISCO_PILOTO SET ENVIA_RISCO = 'NÃO', JOB_RISCO = 0 WHERE ORDEM_SERVICO = '<<xordem_servico>>'
						ENDTEXT
						
						xContUpdt = 1
						DO while !f_update(xUpdateEnviaRisco0)  AND xContUpdt <= 5	
							IF xContUpdt = 5	
								messagebox("Não foi possível Atualizar Dados na tabela GS_ENVIA_RISCO_PILOTO",48,"Atenção_8!")		
								RETURN .f.
							ENDIF	
					...
					*SE NÃO TEM NO ESTOQUE 330
					IF CurContemPilotoEstoque.tipo_movimentacao = 0 OR RECCOUNT("CurContemPilotoEstoque") = 0
						...
						f_select(xSelContemFat, "CurContemFat",ALIAS())
							...
							ELSE
								TEXT TO xUpdateEnviaPiloto TEXTMERGE NOSHOW
									UPDATE GS_ENVIA_RISCO_PILOTO SET ENVIA_PILOTO = 'NÃO', JOB_PILOTO = 0 WHERE ORDEM_SERVICO = '<<xordem_servico>>'
								ENDTEXT
							...
							ENDIF
					ELSE	
						**VERIFICA SE A OS É DA FASE DE CORTE 015 E 127 E SE O RECURSO É INTERNO
					*Alteração solicitada pelo Daniel no dia 11 de Abril de 2018 -- verificar se na op contém 015 e 127, ao invés de verificar na OS
...
*!*							TEXT TO xSelFaseRecurso NOSHOW TEXTMERGE
*!*								select distinct a.ordem_servico, b.ORDEM_PRODUCAO, pt.fase_producao, pr.tipo_recurso, b.tarefa
*!*								from PRODUCAO_ORDEM_SERVICO a 
*!*								join producao_os_tarefas b
*!*								on a.ordem_servico = b.ordem_servico
*!*								join producao_tarefas pt
*!*								on pt.tarefa=b.tarefa
*!*								join producao_recursos pr
...
*!*							ENDTEXT
				...
				** Lucas Miranda - 06/02/2018
					...
					If UPPER(LTRIM(RTRIM(curselPermCons.valor_propriedade))) == 'NÃO' or f_vazio(curselPermCons.fase_producao)
						MESSAGEBOX("Fase não permitida para envio de consumo !!!",0+16)
						SELECT v_producao_os_01
						replace v_producao_os_01.anm_status_almox with xOldStatusAlmox
						thisformset.lx_form1.cmb_status_entrada.enabled=.F.
						thisformset.lx_form1.lx_pageframe1.page6.lx_pageframe1.page1.lx_GRID_FILHA1.col_tx_COR_MATERIAL_SUBSTITUIDO.enabled = .f.
					...
					**** Marco Banaggia - Card Aviso de Aviamento ao enviar OS para Consumo
					text to xselos noshow textmerge
						SELECT count(*) qtde_avi
						FROM PRODUCAO_RESERVA A
							JOIN MATERIAIS B ON B.MATERIAL=A.MATERIAL 
						WHERE ANM_RESERVA_ALMOX=1 AND B.TIPO LIKE '%AVI%' and ORDEM_PRODUCAO = '<<v_producao_os_01_tarefas.ordem_producao>>' 
					endtext
					f_select(xselos, "curselOs", alias())
					
					IF alLtrim(v_producao_os_01.FASE_PRODUCAO) $ ALLTRIM(THISFORMSET.PP_GS_CODIGO_FASES_CORTE)
						if curselOs.qtde_avi > 0
							if curselOs.qtde_avi = 1
								xMsg=messagebox('Existe aviamento marcado como separa na fase ' + alLtrim(v_producao_os_01.desc_FASE_PRODUCAO) + ',' + CHR(13) + 'Confirma o envio ?',48+4,'Atenção')
							endif
							if curselOs.qtde_avi > 1
								xMsg=messagebox('Existem aviamentes marcados como separa na fase ' + alLtrim(v_producao_os_01.desc_FASE_PRODUCAO) + ',' + CHR(13) + 'Confirma o envio ?',48+4,'Atenção')
							ENDIF
							if xMsg = 7
								SELECT v_producao_os_01
								replace v_producao_os_01.anm_status_almox with xOldStatusAlmox
					...
					** #1 - Julio Cesar - 11/05/2016 - CONTROLE P/ NÃO ENVIAR MAT PARA ALMOX COM CUSTO IGUAL 0
					** Incio **
							...
							IF !f_vazio(xCurVerifCorte.tem_corte)
								IF MESSAGEBOX('OP com fase de corte interno,'+CHR(13)+'Deseja alterar a filial da OS para: '+;
								   ...
								   if !f_update("Update PRODUCAO_ORDEM_SERVICO Set FILIAL = ?xCurVerifCorte.filial WHERE ORDEM_SERVICO = ?v_producao_os_01.ordem_servico")
								   		MESSAGEBOX("Não foi possível alterar a filial da OS.",64)
								   		replace v_producao_os_01.anm_status_almox with xOldStatusAlmox
										thisformset.lx_form1.cmb_status_entrada.enabled=.F.
										thisformset.lx_form1.lx_pageframe1.page6.lx_pageframe1.page1.lx_GRID_FILHA1.col_tx_COR_MATERIAL_SUBSTITUIDO.enabled = .f.
										thisformset.lx_form1.lx_pageframe1.page6.lx_pageframe1.page1.lx_GRID_FILHA1.col_tx_COR_MATERIAL_SUBSTITUIDO.Refresh()
				...
				**** Verificando se Filial da OS é igual a filial do ultimo consumo ****
				...
				IF !f_vazio(xCurVerifFilialOS.FILIAL)
					...
					xRespConsumo = MESSAGEBOX('Já existe um consumo na filial: '+ALLTRIM(xCurVerifFilialOS.FILIAL)+CHR(13)+;
				...
				**** Fim da verificações ****
				...
				IF xVerifReserva.QTDE = 0
					MESSAGEBOX("ATENÇÃO!"+CHR(13)+"Impossível Enviar OS, Não existe material a ser consumido.",64)
					replace v_producao_os_01.anm_status_almox with xOldStatusAlmox
					thisformset.lx_form1.cmb_status_entrada.enabled=.F.
					thisformset.lx_form1.lx_pageframe1.page6.lx_pageframe1.page1.lx_GRID_FILHA1.col_tx_COR_MATERIAL_SUBSTITUIDO.enabled = .f.
					thisformset.lx_form1.lx_pageframe1.page6.lx_pageframe1.page1.lx_GRID_FILHA1.col_tx_COR_MATERIAL_SUBSTITUIDO.Refresh()
				...
				*ORDEM_SERVICO,ANM_STATUS_ALMOX,DATA_ALTERACAO,USUARIO_LINX,USUARIO_WINDOWS,NOME_MAQUINA,TIMESTAMP
				text to	xinsert1 noshow textmerge	
					...
					SELECT 
					'<<xordem_servico>>',
					'<<v_producao_os_01.ANM_STATUS_ALMOX>>',(SELECT CAST(GETDATE() AS DATE)),
...
DEFINE CLASS tx_gs_lote AS lx_textbox_base 
	...
	ControlSource = "V_PRODUCAO_OS_01.gs_lote_os"
	Height = 19
	Left = 350
	TabIndex = 11
	Top = 153
