...
define class obj_entrada as custom
	*- Nome do metodo/função que os objetos linx vão chamar.
	...
	procedure metodo_usuario
		...
		*=messagebox( 'Metodo '+ UPPER(xmetodo) + ' Nome Objeto ' + upper(xnome_obj) )
			...
			do case
				...
				***	#1 - ANM.LUCAS.MIRANDA - 22/03/2016 - INCLUIDA A OPÇÃO "SEM OCORRENCIA"
						text to xsel_ocorrencia noshow textmerge	
							select convert(varchar(50),'') as desc_ocorrencia,'' as ocorrencia
							union all
							select convert(varchar(50),'Sem Ocorrência') as desc_ocorrencia,'88' as ocorrencia
							union all
							select desc_ocorrencia,ocorrencia from ctb_ocorrencia
						endtext
				***	#1 - ANM.LUCAS.MIRANDA	-- FIM

						f_select(xsel_ocorrencia ,"curOcorrencias",alias())
					
 
			             with thisformset.lx_filtro_bordero.frame_tipo_titulo.page_a_receber_cheque.frame.principal
 								
								** realocando objetos para caber combo customizado
								.label_conta_portador.top=144
								.label_conta_portador.anchor = 0
								.tx_conta_portador.top=157
								.tx_conta_portador.anchor = 0
								.tx_desc_conta_portador.top=157
								.tx_desc_conta_portador.anchor = 0
								
								.addObject("cmb_acompanhamento","lx_combobox")
								.cmb_acompanhamento.visible=.t.
								.cmb_acompanhamento.top=123
								.cmb_acompanhamento.left=9
								.cmb_acompanhamento.width=199
								...
								.addObject("ck_cmc7","lx_checkbox")
								.ck_cmc7.visible=.t.
								.ck_cmc7.top = 6
								.ck_cmc7.left = 65
								.ck_cmc7.Caption = "Validar CMC7"
								.ck_cmc7.Anchor = 0
					...
					BINDEVENT(thisformset.lx_filtro_bordero.cmdlocalizaragora,'CLICK',This,'MIT_VALIDA_CMC7',1)
	...
	procedure MIT_VALIDA_CMC7

		if thisformset.lx_filtro_bordero.frame_tipo_titulo.page_a_receber_cheque.frame.principal.ck_cmc7.value = 1
			select (thisformset.lx_filtro_bordero.p_controlsource_filtro)
			delete for f_vazio(CMC7_CVCARTAO) or at('-',cmc7_cvcartao) != 0
			select (thisformset.lx_filtro_bordero.p_controlsource_filtro)
			go top
			
			thisformset.lx_filtro_bordero.lx_msg.l_mensagem('Concluído.')
			thisformset.lx_filtro_bordero.lx_msg_result.l_mensagem( thisformset.lx_filtro_bordero.l_totalizar() )
			thisformset.lx_filtro_bordero.Refresh()
