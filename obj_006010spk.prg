		...
		SET STEP ON
			...
			Case UPPER(ALLT(xmetodo)) == 'USR_INIT'
				*SET STEP ON 
				...
				THISFORMSET.lx_FORM1.lx_PAGEFRAME1.Page1.LX_GRID_BASE1.COL_tx_SETOR_PRODUCAO.ColumnOrder=11
				THISFORMSET.lx_FORM1.lx_PAGEFRAME1.Page1.LX_GRID_BASE1.COL_tx_SETOR_PRODUCAO.H_tx_SETOR_PRODUCAO.Caption= 'NF Entrada'
				
				*ALTERAÇÃO CAMPO QTDE_ESTOQUE_AUX - QTDE ORIGINAL/FRACIONADA PRA SEMPRE TRAZER 0 PARA NÃO DUPLICAR
				TEXT TO xSelectCmd NOSHOW TEXTMERGE
					
					SELECT DISTINCT 
						ESTOQUE_MAT_PECA.PECA, ESTOQUE_MAT_PECA.PARTIDA, ESTOQUE_MAT_PECA.MATERIAL, ESTOQUE_MAT_PECA.FILIAL, ESTOQUE_MAT_PECA.COR_MATERIAL,
						WMS_LOCALIZACAO.LOCALIZACAO AS LOCALIZACAO, ESTOQUE_MAT_PECA.QTDE_AUX, 
						ESTOQUE_MAT_PECA.QTDE, ESTOQUE_MAT_PECA.LARGURA, ESTOQUE_MAT_PECA.DATA_ULT_ENTRADA,   ESTOQUE_MAT_PECA.DATA_ULT_SAIDA, ESTOQUE_MATERIAIS.QTDE_ESTOQUE, 0 AS QTDE_ESTOQUE_AUX, MATERIAIS.TIPO, 
						MATERIAIS.FASE_PRODUCAO,  'NF:'+RTRIM(ENTRADAS_MAT_PECA.NF_ENTRADA) AS SETOR_PRODUCAO,
						MATERIAIS.FABRICANTE, MATERIAIS.GRUPO, MATERIAIS.COLECAO, MATERIAIS.SUBGRUPO, MATERIAIS.DESC_MATERIAL,  MATERIAIS.UNID_ESTOQUE, MATERIAIS.DIAS_ENTREGA, 
						MATERIAIS.UNID_AUXILIAR, --MATERIAIS.UNID_AUXILIAR, 
						MATERIAIS.LARGURA AS LARGURA_MATERIAL,  MATERIAIS_CORES.DESC_COR_MATERIAL, MATERIAIS_COMPOSICAO.DESC_COMPOSICAO, MATERIAIS_CORES.REFER_FABRICANTE,   
						MATERIAIS.CTRL_UNID_AUX, FILIAIS.EMPRESA, 
						ESTOQUE_MAT_PECA.COD_CLIFOR , ESTOQUE_MAT_PECA.RECEITA, CADASTRO_CLI_FOR.NOME_CLIFOR   

					FROM ESTOQUE_MAT_PECA   
						INNER JOIN FILIAIS ON (ESTOQUE_MAT_PECA.FILIAL = FILIAIS.FILIAL )  
						INNER JOIN ESTOQUE_MATERIAIS 
							ON (ESTOQUE_MAT_PECA.FILIAL = ESTOQUE_MATERIAIS.FILIAL AND ESTOQUE_MAT_PECA.MATERIAL = ESTOQUE_MATERIAIS.MATERIAL AND ESTOQUE_MAT_PECA.COR_MATERIAL = ESTOQUE_MATERIAIS.COR_MATERIAL )  
						INNER JOIN MATERIAIS 
							ON (ESTOQUE_MAT_PECA.MATERIAL = MATERIAIS.MATERIAL )  
						INNER JOIN MATERIAIS_COMPOSICAO ON (MATERIAIS.COMPOSICAO = MATERIAIS_COMPOSICAO.COMPOSICAO )  
						INNER JOIN MATERIAIS_CORES 
							ON (ESTOQUE_MAT_PECA.MATERIAL = MATERIAIS_CORES.MATERIAL AND ESTOQUE_MAT_PECA.COR_MATERIAL = MATERIAIS_CORES.COR_MATERIAL )  
						LEFT JOIN CADASTRO_CLI_FOR ON (ESTOQUE_MAT_PECA.COD_CLIFOR = CADASTRO_CLI_FOR.CLIFOR ) 
						LEFT JOIN (select peca,entradas.emissao, entradas.nf_entrada, SUM(entradas_mat_peca.QTDE) as QTDE_FRACIONADA 
										from entradas, entradas_mat_peca 
									where entradas_mat_peca.NF_ENTRADA = entradas.NF_ENTRADA and ENTRADAS_MAT_PECA.NOME_CLIFOR = entradas.NOME_CLIFOR and entradas_mat_peca.peca <> 'ajuste'  
								   group by peca,entradas.emissao, entradas.nf_entrada ) ENTRADAS_MAT_PECA 
							ON ESTOQUE_MAT_PECA.PECA = ENTRADAS_MAT_PECA.PECA 
						LEFT JOIN ( SELECT ID_LOG.*,CASE WHEN LOG_LOC.PROCESSO = 'DESLOCALIZACAO PICKING MP' THEN 'TRANSITO' 
						                                 WHEN LOG_LOC.PROCESSO LIKE '%DESLOCALIZACAO%' AND LOG_LOC.PROCESSO <> 'DESLOCALIZACAO PICKING MP' THEN NULL 
												    ELSE LOG_LOC.LOCALIZACAO END AS LOCALIZACAO 
									FROM (SELECT MAX(ID) AS ID,PECA,MATERIAL,COR_MATERIAL FROM SS_LOG_LOCALIZACAO_MP (NOLOCK) A	JOIN MATERIAIS_LOCALIZA (NOLOCK)  B ON A.LOCALIZACAO = B.LOCALIZACAO 
									WHERE  PROCESSO NOT IN ('LOCALIZACAO PICKING CAIXA','LOCALIZACAO DE CAIXA','LOCALIZACAO DE /PARA ID PICKING','DESLOCALIZACAO DE /PARA ID PICKING') 
										AND QTDE > 0
									GROUP BY PECA,MATERIAL,COR_MATERIAL ) ID_LOG 
								    JOIN SS_LOG_LOCALIZACAO_MP  (NOLOCK) LOG_LOC 
										ON ID_LOG.ID = LOG_LOC.ID AND ID_LOG.MATERIAL = LOG_LOC.MATERIAL AND ID_LOG.COR_MATERIAL = LOG_LOC.COR_MATERIAL 
								   ) WMS_LOCALIZACAO 
							ON (ESTOQUE_MAT_PECA.PECA = WMS_LOCALIZACAO.PECA AND ESTOQUE_MAT_PECA.MATERIAL = WMS_LOCALIZACAO.MATERIAL AND ESTOQUE_MAT_PECA.COR_MATERIAL = WMS_LOCALIZACAO.COR_MATERIAL ) 
									
				ENDTEXT

				*THISFORMSET.DataEnvironment.Cur_v_estoque_pecas_00.SelectCmd=x+x1+x2+x3+x4+x5+x6+x7+x8+x9+x10+x11+x12+x13+x14
				THISFORMSET.DataEnvironment.Cur_v_estoque_pecas_00.SelectCmd=xSelectCmd
