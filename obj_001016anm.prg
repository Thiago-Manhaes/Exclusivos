		...
		*=messagebox( 'Metodo '+ UPPER(xmetodo) + ' Nome Objeto ' + upper(xnome_obj) )
			...
			do case
				...
				xalias=alias()
					...
					**** Fornecedores vs Prazo de Pagamento - Lucas Miranda
					If v_fornecedores_01.fornece_materiais = .T. OR v_fornecedores_01.fornece_prod_acab = .T. OR v_fornecedores_01.beneficiador = .T.
						If USED("curCondPgto")
							USE IN curCondPgto
						Endif
						
						If USED("xCondFornecAnt")
							USE IN xCondFornecAnt
						Endif
						...
						Endif
							...
							f_select("select * from COND_ENT_PGTOS (nolock) where numero_parcelas=1 and parcela_1 >= 0 and condicao_pgto=?v_fornecedores_01.condicao_pgto","curCondPgto")
							Sele curCondPgto
							If RECCOUNT("curCondPgto") > 0
								If Thisformset.p_tool_status  $ 'I'	
									If curCondPgto.parcela_1 < Thisformset.pp_gs_minimo_dias_cond_pgto
										f_select("select * from FX_ANM_PARAMETROS_REDE_LOJAS('GS_TIPO_FORNECEDOR_N_VALI')","CurParNVa")
										Sele CurParNVa
										Go Top
										Scan
											If v_fornecedores_01.tipo <> CurParNVa.valor_atual										
												MESSAGEBOX("Somente é permitido condição de pgto maior ou igual à "+ALLTRIM(STR(Thisformset.pp_gs_minimo_dias_cond_pgto)) +;
															" Dias !!!",0+16)
							...
							If Thisformset.p_tool_status  $ 'A'
								...
								Text TO xSelCondPgto TextMerge Noshow
									select b.PARCELA_1,A.* from  FORNECEDORES (nolock) A 
									JOIN COND_ENT_PGTOS (nolock) B 
									ON A.CONDICAO_PGTO = B.CONDICAO_PGTO
									where NUMERO_PARCELAS = 1 
									and parcela_1 >= 0 
									and A.FORNECEDOR= '<<v_fornecedores_01.FORNECEDOR>>'
								Endtext	
								f_select(xSelCondPgto,"xCondFornecAnt")
								
								If RECCOUNT("xCondFornecAnt")>0 AND RECCOUNT("curCondPgto") > 0
									If curCondPgto.parcela_1 != xCondFornecAnt.parcela_1
										If curCondPgto.parcela_1 < xCondFornecAnt.parcela_1 AND Thisformset.pp_gs_perm_alter_p_menor = .F.
											f_select("select * from FX_ANM_PARAMETROS_REDE_LOJAS('GS_TIPO_FORNECEDOR_N_VALI')","CurParNVa")
											Sele CurParNVa
											Go Top
											Scan
												If v_fornecedores_01.tipo <> CurParNVa.valor_atual
													MESSAGEBOX("Somente é permitido condição de pgto maior que a Anterior !!!",0+16)
					...
					*Projeto depara fornecedor materiais
					*Lucas Miranda - 04/07/2017
					If thisformset.p_tool_status $ 'IA'
						If v_fornecedores_01.inativo = .t. 
							if used("CurFabMaterial") 
								use in CurFabMaterial
							endif
							f_select("Select top 1 * from materiais where fabricante=?v_fornecedores_01.fornecedor","CurFabMaterial")
							If RECCOUNT()>0
								MESSAGEBOX("Favor verificar/alterar o fornecedor no cadastro de material !!!",0+16)
								Return .F.
							Endif
						Endif
					Endif
				...
				xalias=ALIAS()
					...
					*** Propriedade 00510 - Fornecedor Internacional
					*** Lucas Miranda - 19/09/2017
					If Thisformset.p_tool_status $ 'I' AND v_fornecedores_01.uf == 'EX' 	
						
						TEXT TO xupdate	NOSHOW TEXTMERGE
							UPDATE PROP_FORNECEDORES SET VALOR_PROPRIEDADE = 'SIM' 
							WHERE PROPRIEDADE = '00510' 
							AND FORNECEDOR = '<<v_fornecedores_01.FORNECEDOR>>'
						ENDTEXT
						
						If !f_update(xupdate)
							MESSAGEBOX("Não foi possível atualizar Fornecedor Internacional !",0+64)
						Else
							Sele CurPropFornecedores
							Locate For CurPropFornecedores.propriedade = '00510'

							Replace CurPropFornecedores.valor_propriedade With 'SIM'
						Endif	
					Endif	
