		...
		*!* Chama a função disponível na classe criada para tratar rede loja produto *!* LUCAS MIRANDA 19/12/2017
	 	TRY 							
			If TYPE("Thisformset.pp_gs_obj_classes_rl")<>'U' AND Thisformset.pp_gs_obj_classes_rl
				If ! o_gs.l_ver_rede_loja_prod('o_100101',xmetodo, xobjeto ,xnome_obj,Thisformset.p_tool_status)
					RETURN .f.
				Endif
			Endif
		CATCH					
...
*!*						endif
				...
				*!* Chama a função disponível na classe criada para tratar rede loja produto *!* LUCAS MIRANDA 19/12/2017 
					If TYPE("o_100101.pp_gs_obj_classes_rl")<>'U' AND Thisformset.pp_gs_obj_classes_rl 
				...
				case upper(xmetodo) == 'USR_WHEN'
					...
					************ Validação para setar o checkbox Consumidor final, para envio de Mostruarios. 
					***retirado em 08/02/2017 só habilitar após autorização do Rodrigo e lideres dos setores envolvidos.
					*** habilitar a transacao GS_0026%
*!*						IF upper(xnome_obj)=='TV_TIPO_FATURAMENTO'
*!*						
*!*							xNat = thisformset.lx_form1.tv_natureza_saida.value
*!*							xFilial = thisformset.lx_form1.tv_filial.value
*!*							xTipo = thisformset.lx_form1.lx_pageframe1.page1.lx_pageframe1.page1.tv_tipo_faturamento.value
*!*							
*!*							TEXT TO xsql TEXTMERGE NOSHOW 
*!*								SELECT ID_CONTROLE FROM GS_FAT_CONSUMIDOR_FINAL WHERE NATUREZA_OPERACAO='<<xNat>>' AND FILIAL='<<xFilial>>' AND TIPO_FATURAMENTO='<<xTipo>>'						
*!*							ENDTEXT 
*!*							
*!*							f_Select(xsql,"tmpResultado",ALIAS())
*!*							SELECT tmpResultado						
*!*							
*!*							IF !EOF() then
*!*								thisformset.lx_form1.lx_pageframe1.page1.lx_pageframe1.page1.ck_indica_consumidor_final.Value = 1
*!*							ELSE
*!*								thisformset.lx_form1.lx_pageframe1.page1.lx_pageframe1.page1.ck_indica_consumidor_final.Value = 0
*!*							ENDIF
*!*						ENDIF
				...
				case upper(xmetodo) == 'USR_VALID'
					...
					IF upper(xnome_obj)=='TV_NOME_CLIFOR'
						...
						** WRP - 23/03/2017 - Codigo atende necessidade do Julio Cesar apra controle de operações entre filiais
						...
						xcod_natureza = v_faturamento_05.natureza_saida
							...
							IF (xcont2 <> 0)
								...
								IF (xcont_natureza < 1)
									...
									MESSAGEBOX("Está operação não poderá ser realizada com a natureza de operação "+xcod_natureza+" somente com a(s) :"+xnatureza2+"")
									RETURN .f.
								ENDIF
							
							ELSE
								IF (xcod_natureza = '120.04')
									MESSAGEBOX("Está operação não poderá ser realizada com a natureza de operação 120.04")
									RETURN .f.
								endif
							Endif
						...
						** FIM - WRP - 23/03/2017 - Codigo para bloqueio da lei da moda
						...
						IF !f_vazio(TabPreco.filial)
							** WRP - 27/03/2017 - Codigo para setar tabela CT caso natureza de operação 120.04
							IF (xcod_natureza = '120.04' )
