...
<% 
	...
	'---- 06/03/2012 --- Marco Banaggia - Retirada do Caracter ASCII na concatenação do Endereço
	...
	Dim xramal3,xcontato,xemail,xemailNFe,xpais,xddi,xniver, xbanco, xagencia, xncliente, xvalidade, xcontrato, xconta	
    ...
    xemailNFe       = Utrim(Request.Form("EmailNFe"), "A")	
	...
	If xpesquisa = "Pesquisacep" Then
		...
		If not ISclean(xcep) and IsNumeric(xcep) Then
			...
			If SqlQuery( xresult, xsql) Then
				...
				'------ Marco Banaggia - 06/03/2012 - Retirado do Caracter ASCII que era gravado no banco, dando erro na NFe
				'xendereco       = trim(xresult("tipo_log")) & Espacos(1) & xresult("Nome_log")
				xendereco       = trim(xresult("tipo_log")) & " " & xresult("Nome_log")
	...
	If xpesquisa = "Pesquisa" or Not IsClean(xcd) Then
		If  Not IsClean(xcgccpf) Or Not IsClean(xcodigo) Or  Not IsClean(xrazao)  Or Not IsClean(xcliente)  or Not IsClean(xcd) Then
			...
			xstr_sql = xstr_sql & "d.endereco, d.numero, d.complemento, d.cidade, d.bairro, d.email, d.email_nfe, d.cep, d.uf, d.telefone1, d.ddd1, c.transportadora, c.tipo, "
			...
			IF xTotCli = 1 Then
				...
				xemailNFe      	= Trim(xresultCli("email_nfe"))
		...
		If IsClean(xMens) Then		
			...
			If not SqlQuery(xresult, xsql) Then
				...
				If IsClean(xmens)  Then
					...
					xsqla =  xsqla & "ddd2, telefone2, ramal2, fax, dddfax,contato, email, email_nfe, pais, ddi, aniversario, "
					...
					xsqla =  xsqla & "'" & xramal2 & "','" & xfax & "','" & xdddfax & "','" & xcontato & "', '" & xemail & "', '" & xemailNFe & "',"
...
'-- Retirado a Alterção do cadastro via LinxWeb - Marco Banaggia
...
<Tr>
	...
	<%
		...
		Response.Write "<Tr><Td class='tituloDetalhe'  width='100'>" & Lang.F_Traduz("Nome Fantasia") &  "</td>"
...
'--- Marco Banaggia -- Retirada co campo de Alteração do Cadastro de Cliente		
'		If ((xFlag = "Flag") or Not IsClean(xcd)) and Session("Altera_cliente_internet") Then
'			Exibe_Botao_Update()
'		End IF		
'--- Marco Banaggia -- Fim -- Retirada co campo de Alteração do Cadastro de Cliente		
...
<Tr>
	...
	<%
		...
		Response.Write "<Td class='td_verd_bk' align='Left' colspan='3'>"
		Response.Write "<input type='Text' name='email' class='ip_txt' size='50' maxlength='40' value='" & xemail & "' onkeyup = ""Copiar 'email','emailNFe','emailNFe'"" onchange = ""Copiar 'email','emailNFe','emailNFe'""> </td></tr>"
		
		'-- Marco Banaggia - 23/03/2012 - Colocado novo campo de email para NFe
		Response.Write "<Tr><Td class='tituloDetalhe' >" & Lang.F_Traduz("Email NFe") & "</td>"
		Response.Write "<Td class='td_verd_bk' align='Left' colspan='3'>"
		Response.Write "<input type='Text' name='emailNFe' class='ip_txt' size='50' maxlength='40' value='" & xemailNFe & "'</td></tr>"
		'-- Marco Banaggia -- Fim do Novo Campo email para NFe
...
Function Valida()
        ...
        '-- Marco Banaggia - Validação dos campos de email e email NFe
        x_mens = x_mens & Iif(IsClean(xemail) , Lang.F_Traduz("Email em Branco ou valor inválido!!! ") & "<Br>", "")
        x_mens = x_mens & Iif(IsClean(xemailNFe) , Lang.F_Traduz("Email NFe em Branco ou valor inválido!!!") & "<Br>", "")
        '-- Marco Banaggia - Fim da Validação dos campos de email e email NFe		
		x_mens = x_mens & Iif(IsClean(xnumero) , Lang.F_Traduz("Número em Branco!!! ") & "<Br>", "")
