					...
					If upper(xnome_obj) = 'CMB_STATUS_COMPRA' AND Thisformset.p_tool_status $ 'IA' 
						...
						sele CurPropValCusto
		           		If !F_Vazio(CurPropValCusto.valor_propriedade) AND CurPropValCusto.valor_propriedade = 'SIM'
							...
							F_Select("Select * From Compras (NOLOCK) Where tabela_filha='COMPRAS_MATERIAL' and Pedido=?V_Compras_01.pedido and Status_Compra IN ('RC','PS')","CurStatusRC")
							If !F_Vazio(CurStatusRC.pedido) AND (CurStatusRC.Status_Compra <> V_Compras_01.status_compra) AND ALLTRIM(v_compras_01.status_aprovacao) <> 'R'
								f_select("Select COUNT(*) AS NAO_VALIDA from DBO.FX_ANM_PARAMETROS_REDE_LOJAS('GS_VALIDA_TIPO_COMPRA_RC') where VALOR_ATUAL = ?CurStatusRC.tipo_compra","CurParamCusto")
									If O_004002.pp_gs_permite_alt_status_rc = .F. AND CurParamCusto.nao_valida = 0
										Sele V_Compras_01
										...
										Replace V_Compras_01.status_aprovacao with CurStatusRC.status_aprovacao 
								 		O_004002.lx_form1.lx_pageframe1.page1.lx_aprovado.Value=IIF(V_Compras_01.status_aprovacao='E','EM ESTUDO',IIF(V_Compras_01.status_aprovacao='A','APROVADO',IIF(V_Compras_01.status_aprovacao='R','REPROVADO',IIF(V_Compras_01.status_aprovacao='P','PENDENTE DE APROVAÇÃO',''))))
										MESSAGEBOX("Você não tem permissão para alterar o"+CHR(13)+ "Status se estiver como Reprovado - Custo",0+16,"Ops!")
									ENDIF
					...
					if type("thisformset.lx_form1.lx_pageframe1.page8.lb_status_entrada")<>"U"
						...
						** Reprova Custo
						If TYPE('ThisFormSet.lX_FORM1.lx_PAGEFRAME1.page2.lX_GRID_FILHA1.col_material_rc.ck_gs_material_rc')<>'U'
							...
							Else	
								If THISFORMSET.p_tool_status $ 'AP' AND ALLTRIM(V_Compras_01.status_compra) =  'RC'
									ThisFormSet.lX_FORM1.lx_PAGEFRAME1.page2.lX_GRID_FILHA1.col_material_rc.ck_gs_material_rc.Enabled=.T.
									f_select("SELECT * FROM PROP_FORNECEDORES WHERE PROPRIEDADE = '00474' and fornecedor=?v_compras_01.fornecedor","CurPropValCusto")
									sele CurPropValCusto
					           			If !F_Vazio(CurPropValCusto.valor_propriedade) AND CurPropValCusto.valor_propriedade = 'SIM'
