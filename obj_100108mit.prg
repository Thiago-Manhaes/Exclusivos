		...
		IF 6 = MESSAGEBOX('Deseja exportar os RPS das nota selecionadas?',36, wusuario)
			...
			SCAN
				IF v_impressao_nf_00.serie_nf = '92' AND !v_impressao_nf_00.nota_cancelada
					VLC_Emissao = LEFT(dtos(v_impressao_nf_00.emissao),4) + '-' + SUBSTR(dtos(v_impressao_nf_00.emissao), 5,2) + '-' + right(dtos(v_impressao_nf_00.emissao), 2) + 'T' + ALLTRIM(TIME())
					...
					VLC_Select = "select pj_pf, replace(razao_social, '&', 'E') as razao_social, cgc_cpf, indicador_fiscal_terceiro, endereco, numero, complemento, bairro, cod_municipio_ibge, uf, cep from cadastro_cli_for (nolock) where nome_clifor = '" + v_impressao_nf_00.nome_clifor + "'"
					F_select(VLC_Select, 'cur_cli', ALIAS())

					VLC_Select = "select MAX(replace(replace(replace(UPPER(DESCRICAO_ITEM), '-', ''), 'Ç', 'C'), 'Ã', 'A')) as descricao_item from faturamento_item (nolock) where nf_saida = '" + v_impressao_nf_00.nf + "' and filial = '" + v_impressao_nf_00.filial + "' and serie_nf = '" + v_impressao_nf_00.serie_nf + "'"
					F_select(VLC_Select, 'cur_item', ALIAS())
					
					VLC_Select = "select SUM(case when id_imposto = 17 then valor_imposto else 0 end) as valor_pis, " + ;
								 "		SUM(case when id_imposto = 18 then valor_imposto else 0 end) as valor_cofins, " +;
								 "		SUM(case when id_imposto = 4 then valor_imposto else 0 end) as valor_INSS, " +;
								 "		SUM(case when id_imposto = 20 then valor_imposto else 0 end) as valor_IRRF, " +;
					...
					VLC_XML = VLC_XML + '<ValorServicos>' + ALLTRIM(STR(v_impressao_nf_00.valor_sub_itens,10,2)) + '</ValorServicos>'
					...
					VLC_XML = VLC_XML + '<Aliquota>' + ALLTRIM(STR(cur_imp.aliquota_ISS/100,10,2)) + '</Aliquota>'				
					...
					VLC_Macro = "STRTOFILE(VLC_XML, 'C:\temp\NF" + ALLTRIM(v_impressao_nf_00.nf) + ".xml')"
					&Vlc_Macro
			...
			MESSAGEBOX('Arquivos criados com sucesso!', 64, wusuario)
		ENDIF
	ENDPROC
