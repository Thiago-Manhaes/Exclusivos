...
* DATA...........:  20/01/2007  altera��o thiago-Manhaes                                                                                             *
* HORÁRIO........:  13;00                                                                                           *
********************************************************************************************************************* 
* CLIENTE..: Animale                                                                                                *
...
**  Fim - Bloqueio alterar reserva se estiver enviado para almoxarifado Lucas Miranda - 30/11/2016 **	
				...
				case upper(xmetodo) == 'USR_SAVE_BEFORE' 
					...
					If Thisformset.p_tool_status = 'A' AND iif(TYPE("Thisformset.pp_Permite_desprogramar_prod")="U",.f.,.t.)
						...
						xProgNaoValida = 0
						xSelect="select * FROM dbo.FXANM_ConsultaVarString('"+ALLTRIM(Thisformset.pp_GS_PROG_NAO_BLOQ_DESPROG)+"')"
						f_select(xSelect,"CurValidaNomeProg")
						
						GO top
						SCAN					
							IF ALLTRIM(CurValidaNomeProg.valores) $ ALLTRIM(v_producao_ordem_01.programacao)
								xProgNaoValida = 1
							ENDIF
						ENDSCAN
						...
						If xProgNaoValida = 0 AND v_producao_ordem_01.tipo_processo = 0
							...
							Scan	
								TEXT TO xSelOP TEXTMERGE NOSHOW
									...
									WHERE A.ORDEM_PRODUCAO = '<<v_producao_ordem_01_cores.ordem_producao>>'
									AND B.PRODUTO = '<<v_producao_ordem_01_cores.produto>>'
									AND B.COR_PRODUTO = '<<v_producao_ordem_01_cores.cor_produto>>'
								ENDTEXT
										
								f_select(xSelOP,"Cur_ProdOrdem")
								
								Sele Cur_ProdOrdem
								locate FOR  produto = v_producao_ordem_01_cores.produto AND ;
											cor_produto = v_producao_ordem_01_cores.cor_produto AND;
											ordem_producao = v_producao_ordem_01_cores.ordem_producao
