		 ...
		 *=messagebox( 'Metodo '+ UPPER(xmetodo) + ' Nome Objeto ' + upper(xnome_obj) )
			...
			* Sintese Soluções - Tiago Carvalho - CRIADO OBJETO PARA IMPRIMIR ETIQUETA DE LOCALIZAÇÃO DE MATÉRIA PRIMA
			Case UPPER(ALLT(xmetodo)) == 'USR_INIT'
				...
				oCur.addbufferfield("'' AS DATA_FINALIZADO", "C(25)",.T., "DATA_FINALIZADO", "DATA_FINALIZADO")
				...
				** Inclusão do campo data_finalizado Lucas Miranda - 16/02/2018
				with thisformset.lx_form1.lx_pageframe1.page1
					.addobject("lbl_gs_dt_finalizado","lbl_gs_dt_finalizado")
					.addobject("gs_txtbox_dt_finalizado","gs_txtbox_dt_finalizado")
					.gs_txtbox_dt_finalizado.InputMask = "99/99/9999"
				endwith
				** Fim - Inclusão do campo data_finalizado Lucas Miranda
				...
				case upper(xmetodo) == 'USR_CLEAN_AFTER'
					xalias=alias()
						...
						if type("thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado")<>"U"
							Thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.Value = ""
				...
				xalias=alias()
					...
					*** Data Finalizado - Lucas Miranda - 16/02/2018
					if type("thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado")<>"U"
						If thisformset.p_tool_status = "L" 
							thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.enabled=.t.
						Else	
							thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.enabled=.f.
						Endif		
						
						If Thisformset.p_tool_status = "P"
							TEXT TO xDataFP TEXTMERGE NOSHOW
					
								SELECT *, convert(varchar(10),DATA_ALTERACAO,103) AS DATA_FINALIZADO
								FROM ANM_STATUS_OS_MOV 
								WHERE ANM_STATUS_ALMOX LIKE '%FINALIZ%'
								AND CHAVE_SAI_MAT = '<<v_estoque_sai_mat_00.req_material>>'
								AND ORDEM_SERVICO = '<<v_estoque_sai_mat_00.ordem_servico>>' 
								
							ENDTEXT

							f_select(xDataFP,"CurDataFP",ALIAS())
							
							Thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.Value=CurDataFP.data_finalizado
						Endif
					ENdif
				...
				*** Data Finalizado - Lucas Miranda - 16/02/2018
				CASE UPPER(xmetodo)=='USR_SEARCH_BEFORE'
					...
					If Thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.Value <> "  /  /    "
						If ! f_vazio(Thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.Value)
							If f_vazio(Thisformset.P_Pai_Filtro) &&ThisFormset.Lx_form1.Lx_chkbox_encerrado1.value = 1	
								Thisformset.P_Pai_Filtro = Thisformset.P_Pai_Filtro +; 
									" ESTOQUE_SAI_MAT.REQ_MATERIAL IN (SELECT CHAVE_SAI_MAT FROM ANM_STATUS_OS_MOV WHERE ANM_STATUS_ALMOX LIKE '%FINA%'"+;
									" AND DATA_ALTERACAO = '"+;									
									dtos(CTOD(Thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.Value)) + "' ) "
							Else
								Thisformset.P_Pai_Filtro = Thisformset.P_Pai_Filtro + ;
									" AND ESTOQUE_SAI_MAT.REQ_MATERIAL IN (SELECT CHAVE_SAI_MAT FROM ANM_STATUS_OS_MOV WHERE ANM_STATUS_ALMOX LIKE '%FINA%'"+;
									" AND DATA_ALTERACAO = '"+;	
									dtos(CTOD(Thisformset.lx_form1.lx_pageframe1.page1.gs_txtbox_dt_finalizado.Value)) + "' ) "
							Endif
						ENDIF
	          	...
	          	*** Fim - Data Finalizado - Lucas Miranda - 16/02/2018	
				...
				SCAN
					...
					TEXT TO xDataF TEXTMERGE NOSHOW
					...
					f_select(xDataF,"CurDataFinal",ALIAS())
					
					sele V_ESTOQUE_SAI_MAT_00
					
					replace data_finalizado       WITH CurDataFinal.data_finalizado					
				ENDSCAN	
...
*!*							
*!*							***********************************************************
*!*							xgsCompras = ThisFormset.LX_FORM1.LX_pageframe1.Page2.LX_GRID_FILHA1.COL_ORDEM_PRODUCAO.TV_ORDEM_PRODUCAO.Value
*!*							TEXT TO UdtVoltaStatusPed TEXTMERGE NOSHOW
*!*							
*!*								update compras set
*!*								anm_status_almox = '1-ENVIADO PARA ALMOX'
*!*								where pedido = '<<xgsCompras>>'
...
EndDefine
**************************************************
*-- Class:        lbl_gs_dt_finalizado (o:\ti\lucas miranda\linx_sql_fonte\lucas\lbl_gs_dt_finalizado.vcx)
*-- ParentClass:  lx_label (c:\program files (x86)\common files\linx sistemas\desenv\lib\lx_class.vcx)
*-- BaseClass:    label
*-- Time Stamp:   02/16/18 02:09:07 PM
*
DEFINE CLASS lbl_gs_dt_finalizado AS lx_label
	...
	TabIndex = 56
	ZOrderSet = 9
	Name = "lbl_gs_dt_finalizado"
	Visible = .T.

ENDDEFINE
*
*-- EndDefine: lbl_gs_dt_finalizado
**************************************************
**************************************************
*-- Class:        gs_txtbox_dt_finalizado (o:\ti\lucas miranda\linx_sql_fonte\lucas\gs_txtbox_dt_finalizado.vcx)
*-- ParentClass:  lx_textbox_base (c:\program files (x86)\common files\linx sistemas\desenv\lib\lx_class.vcx)
*-- BaseClass:    textbox
*-- Time Stamp:   02/16/18 02:10:08 PM
*
DEFINE CLASS gs_txtbox_dt_finalizado AS lx_textbox_base
	...
	Width = 85
	ZOrderSet = 8
	Name = "gs_txtbox_dt_finalizado"
	Visible = .T.

ENDDEFINE
*
*-- EndDefine: gs_txtbox_dt_finalizado
**************************************************