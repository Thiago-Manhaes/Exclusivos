		...
		If Upper(xnome_obj)=='LX_ENTRADA_REVISAO'
			...
			sele xFilialRevisao
				...
				If f_vazio(xFilialRevisao.Filial)
					Messagebox("Filial Revisao Não Encontrada."+CHR(13)+"Não foi Possível Selecionar a Revisão."+CHR(13)+"Favor verificar !!",0+48)
					thisformset.Lx_form1.Lx_pageframe1.page1.pageframe1.page1.lx_entrada_revisao.Value = 0
					Return .F.
				Else
					Sele v_entradas_00_ret1_mat
	...
	Case Upper(xmetodo) == 'USR_SAVE_AFTER'
		...
		*** Entrega Direta ***
		...
		If !F_Vazio(xMsgEntregaDireta)
			MESSAGEBOX("Esse(s) pedido(s) é(são) uma entrega direta: "+LEFT(xMsgEntregaDireta,LEN(xMsgEntregaDireta)-2),0+64)
		Endif	
		*** Fim Entrega Direta ***
		...
		Endif
						...
						************INICIO CUSTO MEDIO MATERIAIS*********************	
						If v_entradas_00.tipo_operacao='C' OR v_entradas_00.natureza='201.01'
									...
									*** Inclusão feito Por Julio para não calcular o custo quando o pedido de compra for mostruário **** Giselli e Patricia (Farm)
									*** 11-03-2015 19:36
...
Define Class btn_importa_peca As botao
	...
	Procedure Click
		If ! ThisFormSet.p_Tool_Status $ 'I,A'
			MESSAGEBOX("Somente pode Importar em modo Inclusão !!!",0+16)
			Return .F.
		EndIf
