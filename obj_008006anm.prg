			...
			do case
				...
				case upper(xmetodo) == 'USR_ITEN_INCLUDE_BEFORE'
					...
					If !Found() Or F_VAZIO(VALOR_PROPRIEDADE)
						Messagebox("Favor selecionar o tipo da programação.",16)
						Return .F.
					Endif
						...
						IF upper(xnome_obj) = 'CK_MATAR_SALDO_PROGRAMACAO'
							If xImport = 1
								...
								IF v_producao_programa_00_prod.qtde_em_op = v_producao_programa_00_prod.qtde_programada ;
									...
									MESSAGEBOX('Não é permitido matar saldo desse Produto/Cor: '+ALLTRIM(v_producao_programa_00_prod.produto)+'-'+ALLTRIM(v_producao_programa_00_prod.cor_produto)+CHR(13)+;
											   'OP ou pedido de compra já emitido.'+CHR(13)+CHR(13)+;
											   'OPERAÇÃO CANCELADA !!!',16,'Matar Saldo')
								...
								IF v_producao_programa_00_prod.qtde_em_op > 0 AND v_producao_programa_00_prod.matar_saldo_programacao = .f.
									...
									MESSAGEBOX('OP ou pedido de compra já emitido do produto/Cor: '+ALLTRIM(v_producao_programa_00_prod.produto)+'-'+ALLTRIM(v_producao_programa_00_prod.cor_produto)+CHR(13)+;
											   'Não é permitido voltar com o saldo.'+CHR(13)+CHR(13)+;
											   'OPERAÇÃO CANCELADA !!!',16,'Matar Saldo')
						...
						*** PROJETO REPROGRAMACAO QUINZENAL
						IF upper(xnome_obj) = 'TX_ENTREGA_INICIAL' AND Thisformset.p_tool_status = 'I'
							...
							If FOUND() AND ppRL_ANM_BLOQ_ENTR_FINAL_PROG.valor_atual = 'SIM'
								...
								If F_Vazio(CurPropProducaoPrograma.valor_propriedade)
									Messagebox("Favor Preencher o Tipo de Programação!!!",0+16,"Tipo Programação Inválido")
									Return .F.
								Else
						...
						IF upper(xnome_obj) = 'TX_ENTREGA_INICIAL'
							***************
							*!* Inicio - Card: Trava de programações
							...
							***** Pega o valor do parametro para o bloqueio de entrega para mostruario
								...
								If Thisformset.pp_GS_DIAS_BLOQ_PROG_MOST > 0 AND xFiltro 
									...
									If vtmp_bloqprog.prazo = 1
										Messagebox("Data não permitida para mostruário"+ Chr(13)+;
											'Data Sugerida: '+dtoc(vtmp_bloqprog.data_programacao_sugerida),48+0,"Informação")
								...
								If Thisformset.pp_GS_DIAS_BLOQ_PROG_PROD > 0
									...
									If Thisformset.p_tool_status = 'I'
										...
										If V_Producao_Programa_00_Prod.ENTREGA_INICIAL  < xDataLimiteProd
											Messagebox("Data não permitida para o produto"+Chr(13)+;
												'Data Sugerida: '+dtoc(xDataLimiteProd),48+0,"Informação")
									...
									Else
										If Thisformset.p_tool_status = 'A'
											...
											If (V_Producao_Programa_00_Prod.ENTREGA_INICIAL < vCurDataLImite.ENTREGA_INICIAL) Or xCountCur=0
												...
												If V_Producao_Programa_00_Prod.ENTREGA_INICIAL < xDataLimiteProd
													If xCountCur > 0
														Messagebox("Não é possível reduzir a data programada"+Chr(13)+;
															IIF(vCurDataLImite.ENTREGA_INICIAL - Thisformset.pp_GS_DIAS_BLOQ_PROG_PROD > DATE(),;
																"Mínima data permitida: "+DTOC(xDataLimiteProd),;
															    "Só é permitido aumentar a data de entrega."),48+0,"Data fora do Prazo")
						...
						ENDIF
						***************
						*!* Fim - Card: Trava de programações
						*!* Marco Banaggia - 30/08/2018
						***************
						...
						*** Lucas Miranda- 25/10/2017
						If TYPE("Thisformset.lx_form1.btn_importa_prog") <> "U"
							If Thisformset.p_tool_status = 'I' 
								...
								Thisformset.lx_form1.btn_importa_prog.caption='Incluir Programação'
							Else   
								If Thisformset.p_tool_status = 'A'
									Thisformset.lx_form1.btn_importa_prog.enabled=.T.
									Thisformset.lx_form1.btn_importa_prog.caption='Alterar Programação'
								Else
									Thisformset.lx_form1.btn_importa_prog.enabled=.F.
									Thisformset.lx_form1.btn_importa_prog.caption='Importar Programação'
								Endif	
							Endif
						Endif
	...
	PROCEDURE Click
		...
		try
						...
						IF  upper(alltrim(objsheet.cells(1,1).text)) <> "PROGRAMACAO" OR ;
							...
							MESSAGEBOX("O Layout do Arquivo é Inválido, O Cabeçalho deve Conter as Seguintes Informações:"+CHR(10)+"PROGRAMACAO Célula A"+CHR(10)+;
										"FILIAL Célula B"+CHR(10)+"COLECAO Célula C"+CHR(10)+"REDE_LOJAS Célula D"+CHR(10)+"LIBERADO Célula E"+CHR(10)+;
										"TIPO_PROGRAMACAO Célula F"+CHR(10)+"BLOQUEIA Célula G"+CHR(10)+"PRODUTO Célula H"+CHR(10)+"COR_PRODUTO Célula I"+CHR(10)+"ENTREGA Célula J"+CHR(10)+;
										"LIMITE Célula K"+CHR(10)+"QTDE Célula L"+CHR(10)+"T1 Célula M"+CHR(10)+"T2 Célula N"+CHR(10)+;
										"T3 Célula O"+CHR(10)+"T4 Célula P"+CHR(10)+"T5 Célula Q"+CHR(10)+"T6 Célula R"+CHR(10)+;
										"T7 Célula S"+CHR(10)+"T8 Célula T"+CHR(10)+"T9 Célula U"+CHR(10)+"T10 Célula V"+CHR(10)+;
										"T11 Célula W"+CHR(10)+"T12 Célula X"+CHR(10)+"T13 Célula Y"+CHR(10)+"T14 Célula Z"+CHR(10)+;
										"T15 Célula AA"+CHR(10)+"T16 Célula AB"+CHR(10)+"ENTREGA_NOVA Célula AC"+CHR(10)+"LIMITE_NOVA Célula AD"+CHR(10)+;
										"MATAR_SALDO Célula AE",0+16,"Obj - Layout Inválido")
							return .f.
							*GO to oexception
						ENDIF
				...
				*** Incluindo Programação - Modo Incluir
				If o_008006.lx_form1.btn_importa_prog.Caption='Incluir Programação'	
					...
					*** Verifica Datas ****
					...
					If RECCOUNT("CUR_PROG_ENTR") > 0
						xMsgCabe = xMsgCabe + CHR(13)+"- Entrega está em branco!"+CHR(13)
					Endif
					...
					If RECCOUNT("CUR_PROG_LIM") > 0
						xMsgCabe = xMsgCabe + CHR(13)+"- Limite está em branco!"+CHR(13)
					Endif
					*** Fim Verifica Datas ***
			...
			****************************************
			****************************************						
			*** Incluindo Programação - Modo Alterar
				If o_008006.lx_form1.btn_importa_prog.Caption='Alterar Programação'	
				xImport=1
					F_wait("Validando as informações do Cabeçalho, Aguarde...")
					*** Verifica Programação ***
					SELECT DISTINCT PROGRAMACAO FROM cur_import where !f_vazio(PROGRAMACAO) INTO CURSOR CUR_PROG_DUPL
					...
					Scan
						If CUR_PROG_CORR.programacao != v_producao_programa_00.programacao and xProgCorr = 0
							xProgCorr = 1
							xMsgCabe = xMsgCabe + CHR(13)+"- Programação informada não corresponde a programação selecionada !"+CHR(13)
						Endif
					EndScan
					...
					*** Fim - Alteração de grade ***
					
					**** Inclui Produto que não existe na programação ****
					Sele cur_import 	
					DELETE FROM cur_import WHERE f_vazio(PRODUTO)
					...
					If RECCOUNT("vProdNaoExiste") > 0
						...
						****Fim -  Inclui Produto que não existe na programação ****
					Endif
