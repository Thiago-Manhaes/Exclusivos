		...
		*!* Fim - Instancia um objeto da classe "Obj_Entrada_XML" #XML_2015
		...
		do case
			...
			case UPPER(xmetodo) == 'USR_INIT' 
				...
				*GS01 -- Criação do objeto na page de Devolução
				...
				*Criação do objeto na page de Retorno
...
***MOSTRA VALOR PRODUCAO
			...
			case UPPER(xmetodo) == 'USR_REFRESH' 
				...
				xalias=alias()
					...
					**** Atualizar automátio estoque fisico
					**** Lucas Miranda 
												...
												GO TOP
													...
													SCAN
														...
														If (ALLTRIM(v_entradas_00_prod1_ent.filial) <> ALLTRIM(CurFilialOS.Filial)) AND (ALLTRIM(v_entradas_00.filial_matriz_fiscal) = ALLTRIM(xMatriz.matriz_fiscal));
															...
															If xPerg = 0
																xPerg = MESSAGEBOX("Deseja entrar com o Pedido na Revisão ?",4+16)
															ENDIF
...
***MOSTRA VALOR PRODUCAO
			*** Gera OP de acabamento conforme informação do pedido ***
			case UPPER(xmetodo) == 'USR_TRIGGER_AFTER' 
				...
				xalias=alias()
					...
					IF THISFORMSET.P_TOOL_status <> 'E'	
						...
						If xErroGerarOP = 1 
							MESSAGEBOX("Não foi possível gerar a OP de acabamento.",48)
							RETURN .f.
						Endif	
					ENDIF
			...
			case UPPER(xmetodo) == 'USR_SAVE_BEFORE' 
				...
				***** Saida Automática dos materiais comprados para Pilotagem **** #XML_2015
				...
				IF THISFORMSET.P_TOOL_status == 'E'	
					...
					******* Inicio
					******* GS.01 - Tratamento de erro, caso não exista lancamento contabil
					IF !F_VAZIO(V_ENTRADAS_00.CTB_LANCAMENTO) OR V_ENTRADAS_00.CTB_LANCAMENTO = '' && #GS.01#
						...
						IF RECCOUNT() > 0
							MESSAGEBOX('Existe Lancamento de Credito para esta Nota, Favor procurar o Financeiro',48+0,'Atenção')
							RETURN .f. 
						ENDIF
						...
						*******	Fim
						...
						MESSAGEBOX("Atenção, Todos os Seus Dados Serão Armazenados ",48+0,"Atenção!")	
						******* Correção da obrigatoriedade do motivo da exclusão
						******* Antes se tecla-se ESC ou clicava no OK, o Motivo Não era preenchido.
						******* Alterado para no minimo 15 caracteres.	
						...
						IF LEN(ALLTRIM(xmotivo_exclusao)) >= 15						 	
							...
							if !f_insert(xinsert1) 
								messagebox("Não foi possível Excluir a Nota Fiscal ",48+0,"Atenção!")
								retu .f.
							ENDIF
						ELSE
							messagebox("Tamanho do Motivo da Exclusão deve ser superior a 15 caracteres, Verifique!",48+0,"Atenção_9!")
							RETURN .f. 
						ENDIF 
					ENDIF && #GS.01#
					****** Fim
