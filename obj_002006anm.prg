	    ...
	    *=messagebox( 'Metodo '+ UPPER(xmetodo) + ' Nome Objeto ' + upper(xnome_obj) )
		...
		do case
				case UPPER(xmetodo) == 'USR_INIT'
					**** Documentação ****
					*** #GS03 - 21/02/2018 - Lucas Miranda (Grupo Soma): Inclusão das colunas (GS_ETAPA e GS_QTDE_VENDIDA_ETAPA) na aba cores
					*** #GS02 - 20/12/2017 - Lucas Miranda (Grupo Soma): Inclusão do botão para importar tempo de produção para a propriedade 00528
					*** #GS01 - 09/11/2017 - Lucas Miranda (Grupo Soma): Inclusão da coluna GS_NOTA_PROD - Solicitante Roberto Fonseca *** 					
					**** Fim Documentação ****				
					
				 	* 16/12/2014 - Leandro Rocha (SS): Adiciona botão para importar o cadastro de produtos
...
*!*							Endif	
					...
					IF thisformset.pp_anm_preenche_aut_pa = .t.
						if !f_vazio(xGrupo)  
							...
							*thisformset.LX_FORM1.LX_PAGEFRAME1.Page1.pgDadosProdutos.Page1.tx_dESC_PROD_NF.Value = xDescFiscal
							...
							If xcopprop.valor_propriedade = 'PRODUÇAO'
				...
				case UPPER(xmetodo) == 'USR_WHEN' AND (THISFORMSET.P_tool_status == 'I' OR THISFORMSET.P_tool_status == 'A')
					...
					IF upper(xnome_obj)== 'CMB_PROPRIEDADE'
							...
							*** TROCADO NO IF
							IF 	ALLTRIM(THISFORMSET.lx_FORM1.lx_PAGEFRAME1.page_pROPS.pageframe_grupos.page_xyz.grid_produtos.col_tx_valor_propriedade.cmb_propriedade.value) = 'PRODUÇAO'
...
*!*					ENDWITH 
				...
				*** Bloqueio Rede Loja - Lucas Miranda 02/08/2016
				...
				case UPPER(xmetodo) == 'USR_VALID' 
					...
					*#13 - JULIO CESAR - 25/08/2016 - ATUALIZAR TABELA CA - PRODUTOS PARA REVENDA * INÍCIO *
					*Procedimento para atualizar CA quando aterar o preço reposição *				
					...
					******************************************************	
					*Procedimento para atualizar CA quando aterar o custo reposição *	 				
					...
					If upper(xnome_obj)== 'TV_REDE_LOJAS' 
						If thisformset.p_tool_status = 'A'
							...
							If !(LTRIM(RTRIM(xValorparAntes)) == '2' and LTRIM(RTRIM(xValorParNovo)) == '2')
								Messagebox("Não foi possível alterar Rede Lojas !!!",0+16)
								Sele v_produtos_00
...
DEFINE CLASS bt_tabPreco AS botao
	...
	PROCEDURE Click
		...
		If f_vazio(v_produtos_00.rede_lojas)
			MESSAGEBOX("Rede Loja do Produto não está preenchido !!!",0+16,"Rede Loja")
			RETURN .F.
		Endif
		...
		IF this.Caption = "Calcula Custo CT e CA"
			...
			f_prog_bar("Executando Cálculo do custo, aguarde ...")
...
**************************************************
*** LUCAS MIRANDA - 19/07/2016 - MELHORIA CAD ***
*-- Class:        lb_status_cad (c:\aplicações\lb_status_cad.vcx)
...
**************************************************
**************************************************
*-- Class Library:  c:\aplicações\txt_status_cad.vcx
**************************************************
 

**************************************************
*-- Class:        txt_status_cad (c:\aplicações\txt_status_cad.vcx)
...
**************************************************
*-- Class Library:  c:\aplicações\lb_status_most.vcx
**************************************************


**************************************************
*-- Class:        lb_status_most (c:\aplicações\lb_status_most.vcx)
*-- ParentClass:  lx_label (c:\linx spk novo lucas\linx\exclusivos\lx_class.vcx)
*-- BaseClass:    label
...
**************************************************
**************************************************
*-- Class Library:  c:\aplicações\txt_status_most.vcx
**************************************************


**************************************************
*-- Class:        txt_status_most (c:\aplicações\txt_status_most.vcx)
*-- ParentClass:  lx_textbox_base (c:\linx spk novo lucas\linx\exclusivos\lx_class.vcx)
*-- BaseClass:    textbox
...
ELSE
	...
	IF StrRede = '29'
		...
		* Solicita arquivo 
		strPathFile = GETFILE('XLSX;XLS', 'Arquivo:', 'IMPORTAR', 0, 'IMPORTAÇÃO DE PRODUTOS')
		...
		* Cria cursor para importar planilha
		CREATE CURSOR curPlanilhaProdutos (	GRIFFE C(50) NULL, ;
											...
											GRADE C(50) NULL, ; && Esse campo não vem na planilha, será preenchido na validação
											INDICA_ERRO L NULL ; && Esse campo não vem na planilha, será preenchido na validação
											)
		...
		TRY
			...
			* Percorre planilha enquanto tiver valor na primeira coluna 
			DO WHILE !EMPTY(NVL(objExcel.Range("A" + ALLTRIM(STR(intLinha))).Value, ''))
				...
				* Não considera a primeira linha porque é cabeçalho
				IF intLinha = 1
					intLinha = intLinha + 1
					LOOP
				ENDIF
		...
		CATCH TO objErro
			f_wait()
			MESSAGEBOX('Erro ao importar planilha.' + CHR(10) + NVL(objErro.Message, ''), 16, 'ATENÇÃO')
			objExcel.Quit()
			RETURN .F.
		ENDTRY	
		...
		* Verifica se os registros foram importados
		IF RECCOUNT([curPlanilhaProdutos]) = 0 
			MESSAGEBOX('Nenhum registro importado, verifique o arquivo e tente novamente.', 16, 'ATENÇÃO')
			RETURN .F.
		ENDIF

		strErros = ''
		intLinha = 2 && Começa na segunda linha porque a primeira é o cabeçalho na planilha 
		...
		* Realiza Validação
		...
		SCAN
			...
			* Se o tamanho anterior tiver parado em alguma validação já pula para o próximo registro
			...
			* VALIDA GRUPO E SUBGRUPO
			...
			IF !f_select(strSql, 'curValidacao')
				MESSAGEBOX('Erro ao validar dados, tente novamente.', 16, 'ATENÇÃO')
				f_wait()
				RETURN .f.
			ENDIF
			
			IF RECCOUNT('curValidacao') = 0
				strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - Grupo não cadastrado. Grupo: ' + ALLTRIM(curPlanilhaProdutos.GRUPO_PRODUTO) + ' SubGrupo:' + ALLTRIM(curPlanilhaProdutos.SUBGRUPO_PRODUTO) + CHR(13) + CHR(10)
				REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
			ELSE
				IF f_vazio(curValidacao.SUBGRUPO_PRODUTO)
					strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - SubGrupo não cadastrado. Grupo: ' + ALLTRIM(curPlanilhaProdutos.GRUPO_PRODUTO) + ' SubGrupo:' + ALLTRIM(curPlanilhaProdutos.SUBGRUPO_PRODUTO) + CHR(13) + CHR(10)
					REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
				ENDIF
			ENDIF
			...
			* VALIDA TIPO DE PRODUTO
			...
			IF RECCOUNT('curValidacao') = 0
				strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - Tipo de produto não cadastrado. Tipo: ' + ALLTRIM(curPlanilhaProdutos.TIPO_PRODUTO) + CHR(13) + CHR(10)
				REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
			ENDIF
			...
			* VALIDA COLEÇÃO
			...
			IF RECCOUNT('curValidacao') = 0
				strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - Coleção não cadastrada. Coleção: ' + ALLTRIM(curPlanilhaProdutos.COLECAO) + CHR(13) + CHR(10)
				REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
			ENDIF
			...
			* VALIDA GRIFFE
			...
			IF RECCOUNT('curValidacao') = 0
				strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - Griffe não cadastrada. Griffe: ' + ALLTRIM(curPlanilhaProdutos.GRIFFE) + CHR(13) + CHR(10)
				REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
			ENDIF
			...
			* VALIDA LINHA
			...
			IF RECCOUNT('curValidacao') = 0
				strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - Linha não cadastrada. Linha: ' + ALLTRIM(curPlanilhaProdutos.LINHA) + CHR(13) + CHR(10)
				REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
			ENDIF
				
			USE IN curValidacao
				
			* VALIDA CLASSIFICAÇÃO FISCAL
			TEXT TO strSQL TEXTMERGE NOSHOW 
			...
			IF RECCOUNT('curValidacao') = 0
				strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - Classificação Fiscal não cadastrada. Classificação Fiscal: ' + ALLTRIM(curPlanilhaProdutos.CLASSIF_FISCAL) + CHR(13) + CHR(10)
				REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
			ENDIF
			...
			* VALIDA FABRICANTE
			...
			IF RECCOUNT('curValidacao') = 0
				strErros = strErros + 'Registro: ' + ALLTRIM(STR(intLinha)) + ' - Fabricante não cadastrado. Fabricante: ' + ALLTRIM(curPlanilhaProdutos.FABRICANTE) + CHR(13) + CHR(10)
				REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
			ENDIF
			...
			* VALIDA GRADE
			IF f_vazio(curPlanilhaProdutos.grade)
				...
				* Percorre os tamanho do produto atual para saber qual grade será usada
				SELECT curTamanhosProduto
				SCAN
					strTamanhos = strTamanhos + ALLTRIM(curTamanhosProduto.TAMANHO) + ','
						
					* Deleta grades que não atendem ao tamanho do produto
					DELETE A ;
						FROM curGrades A ;
				...
				IF intRecCount = 0
					strErros = strErros + 'Não existe grade cadastrada para atender o produto:' + strProduto + ', nos Tamanhos:' + ALLTRIM(strTamanhos) + CHR(13) + CHR(10)
					REPLACE curPlanilhaProdutos.INDICA_ERRO WITH .T. IN curPlanilhaProdutos
				ELSE 
					GO top in curGrades
			...
			* Se tiver parado em alguma validação marca todos os itens do mesmo produto como itens com erro
			IF IIF(EMPTY(NVL(curPlanilhaProdutos.INDICA_ERRO, '')), .F., curPlanilhaProdutos.INDICA_ERRO) 
				intRegistro = RECNO('curPlanilhaProdutos')
		...
		SCAN FOR !INDICA_ERRO
			...
			* INSER PRODUTO 
 			TEXT TO strSql TEXTMERGE NOSHOW 
				...
				/* Se já existir o produto cadastrado para esse licenciado utiliza o código que havia sido cadastrado */
				...
				BEGIN
					...
					IF @PRODUTO IS NULL
					BEGIN
						RAISERROR('Não foi possível encontrar o sequencial para cadastrar o produto.', 18, 1)  	
					END
				END
		...
		* Se der algum erro cria e abre arquivo com os erros 
		IF f_vazio(strErros)
			MESSAGEBOX('Produtos Cadastrados com sucesso!', 64, 'ATENÇÃO')
		ELSE
			IF !DIRECTORY('C:\TEMP')
				MKDIR 'C:\TEMP'
			ENDIF
			...
			IF intArquivo < 0 
				MESSAGEBOX('Erros encontrados na importação dos produtos.' + CHR(10) + strErros, 16, 'ATENÇÃO')
				RETURN .T.
			ENDIF
	...
	PROCEDURE Click
		...
		try
						...
						IF  upper(alltrim(objsheet.cells(1,1).text)) <> "PRODUTO" OR ;
							...
							MESSAGEBOX("O Layout do Arquivo é Inválido, O Cabeçalho deve Conter as Seguintes Informações:"+CHR(10)+"PRODUTO Célula A"+CHR(10)+;
										"COR_PRODUTO Célula B"+CHR(10)+"NOTA Célula C",0+16,"Obj - Layout Inválido")
							return .f.
							*GO to oexception
						ENDIF
				...
				DELETE FROM cur_import WHERE f_vazio(PRODUTO)
					...
					*** Verifica Programação ***
					...
					If !F_Vazio(xMsgCabe)
						...
						MESSAGEBOX("Não foi possível atualizar o produto, contêm os erros abaixo: "+CHR(13)+xMsgCabe+CHR(13)+;
					...
					If !F_Vazio(xMsgErro)
						IF MESSAGEBOX("FAVOR VERIFICAR NO CAMINHO: "+"'"+"C:\TEMP\import_prod_nota\"+"'"+" OS ERROS!!!"+CHR(13)+xMsgErro+CHR(13)+"Deseja abrir os arquivos ?",4+16,"Importação")=6								
							If RECCOUNT("CurVerProd")>0
								f_wait()
...
DEFINE CLASS btn_import_tmp_producao AS botao
	...
	Height = 25
	Width = 140
	Caption = "Importar Tempo Produção"
	ToolTipText = [Importar Tempo Produção]
	Name = "btn_import_tmp_producao"
	Visible = .T.

	PROCEDURE Click
		If Thisformset.PP_GS_importar_tempo_prod = .F.
			MESSAGEBOX("Você não tem permissão para importar !",0+16,"Tempo Produção")
			Return .F.
		Endif	
		...
		try
						...
						IF  upper(alltrim(objsheet.cells(1,1).text)) <> "PRODUTO" OR ;
							...
							MESSAGEBOX("O Layout do Arquivo é Inválido, O Cabeçalho deve Conter as Seguintes Informações:"+CHR(10)+"PRODUTO Célula A"+CHR(10)+;
										"TEMPO Célula B",0+16,"Obj - Layout Inválido")
							return .f.
							*GO to oexception
						ENDIF
				...
				DELETE FROM cur_import WHERE f_vazio(PRODUTO)
					...
					*** Verifica Programação ***
					...
					F_wait("Atualizando o tempo de produção dos Produtos, Aguarde...")
					...
					f_wait()	
					MESSAGEBOX("Tempo de Produção dos produtos Atualizados !!!",0+16,"Nota Produto")
