							...
							**** Lucas Miranda - 16/03/2017 - Bloqueia Programação *****
							...
							Sele Cur_BloqProd 
								...
								If !F_Vazio(Cur_BloqProd.fornecedor) 
									MESSAGEBOX("Esse fornecedor está bloqueado programação !!!",0+16,"Bloqueia Programação")
									Sele v_compras_01
				...
				case upper(xmetodo) == 'USR_WHEN'					
					xalias=alias()
						...
						IF UPPER(xnome_obj)=='TX_ENTREGA_UNICA' AND Thisformset.p_Tool_Status = 'A'
							...
							xReturn = "DODEFAULT()" && Inicia variável como default do objeto
							&xReturn 				&&Executa o default do objeto antes de começar o tratamento exclusivo	
							
							*** Primeira validação antes de liberar o campo: Se usuário tem permissão de alterar campo ou data = '19860101'
							*** #6 - LUCAS MIRANDA - 10/06/2016 - CRIACAO PARAMETRO PARA LIBERAR CAMPO ENTREGA				 	
							If !(DTOC(v_compras_01_produtos.entrega,1) = '19600101' or thisformset.pp_anm_permit_alt_entrega);
								...
								MESSAGEBOX("Você não tem permissão para altera esse campo.",64,"ALERTA")
								xReturn = ".f."
							Endif	

							** Segunda validação antes de liberar o campo: Se qtde original + cancelado = qtde a entregar **
							*** Validação retirada da Tela para somar a qtde cancelado antes de bloquear o campo
							...
							*** Terceira validação antes de liberar o campo: Se pedido já tem agendamento no portal
							...
							** Retorna valor das validações para o método
							Return &xReturn
						...
						*Bloqueia Programação*
						...
						Go Top
							...
							If !F_Vazio(xMsgBloq)
								MESSAGEBOX('Existe(m) fornecedor(es) bloqueados programação.'+CHR(13)+CHR(13)+"Fornecedor(es): "+LEFT(xMsgBloq,LEN(xMsgBloq)-2),0+16,"Bloqueia Programação")
								USE IN Cur_BloqProd
								Return .F.
							Endif
...
*!*							 endif
					...
					case upper(xmetodo) == 'USR_INCLUDE_AFTER'
						...
						*** Correção, caso a filial esteja em branco
						If f_vazio(v_compras_01.anm_filial_material)
							xFilialMatCons = v_compras_01.anm_filial_material
			...
			if messagebox("Deseja Alterar o Status deste Ped. para "+allt(V_Compras_01.anm_status_almox)+" ?",4+32+256,"Atenção!")=6
				...
				** Lucas Miranda - 16/04/2018 - Bloquear Envio de OS/Pedido com Consumo Pendente a Faturar
				...
				IF !F_Vazio(CurBloqNFConsP.req_material)
					MESSAGEBOX("ATENÇÃO!"+CHR(13)+"Impossível Enviar Ped., Consumo do Pedido com N.F. Pendente !",64)
					f_select("select ANM_STATUS_ALMOX from COMPRAS where PEDIDO=?xordem_servico ","curStatsAlmox",alias())
					thisformset.lx_form1.lx_pageframe1.page7.cmb_status_entrada.value=curStatsAlmox.ANM_STATUS_ALMOX 
