...
********************************************************************************************************************* 
* CLIENTE..: Animale                                                                                                *
* OBJETIVO.: Ajuste no botão que pega informações de desconto ( motivo )  											*
*			view criada - W_ANM_WRP_LOJA_EVENTOS_LOG_Motivo_desconto             									*
********************************************************************************************************************* 
			...
			do case
				...
				case upper(xmetodo) == 'USR_INIT'
					...
					with thisform.lx_PAGEFRAME1.page1	
						...
						.addobject("bt_Desc_Obs","bt_Desc_Obs")						
					endwith	

					
					with thisformset.lx_form1.lx_PAGEFRAME1 
						.PageCount=thisform.lx_PAGEFRAME1.PageCount+1
						.page6.FontName =thisform.lx_PAGEFRAME1.page1.FontName
						.page6.FontSize =thisform.lx_PAGEFRAME1.page1.FontSize
						.page6.Caption="Filtros Custom"
						.page6.addobject("lb_filial","lb_filial")
						.page6.addobject("cmdadd","cmdadd")
						.page6.addobject("cmdaddall","cmdaddall")						
						.page6.addobject("cmdremove","cmdremove")
						.page6.addobject("cmdremoveall","cmdremoveall")							
					endwith	


					addnewobject(thisformset.lx_form1.lx_PAGEFRAME1.page6,"lstsource","lstsource")		
					addnewobject(thisformset.lx_form1.lx_PAGEFRAME1.page6,"lstSelected","lstSelected")	
					addnewobject(thisformset.lx_form1.lx_PAGEFRAME1.page6,"line1","line1")					
				...
				case upper(xmetodo) == 'USR_SEARCH_BEFORE' 
					...
					if thisformset.lx_form1.lx_PAGEFRAME1.page6.lstSelected.listcount>0

						for i = 1 to thisformset.lx_form1.lx_PAGEFRAME1.page6.lstSelected.listcount
							xfiltro_filial= xfiltro_filial + "'"+allt(thisformset.lx_form1.lx_PAGEFRAME1.page6.lstSelected.list(i,2))+"',"
						next
				...
				case upper(xmetodo) == 'USR_SEARCH_AFTER' 
					...
					*PROCEDIMENTO ALTERACAO NUMERO VALE
					o_300027.lx_form1.lx_PAGEFRAME1.page2.LX_grid_filha1.Col_tx_CODIGO_BARRA.Tx_CODIGO_BARRA.Enabled =.T.
					*FIM PROCEDIMENTO ALTERACAO NUMERO VALE

					sele v_loja_venda_02
					go top	
					scan	
						f_prog_bar("Selecionando Filiais das Vendas...",RECNO(),RECCOUNT())
						f_select("select filial from lojas_varejo where codigo_filial=?v_loja_venda_02.codigo_filial","curFilial300012",alias())
						repla filial with curFilial300012.filial 
					endscan	
					f_prog_bar()
					...
					*PROCEDIMENTO ALTERACAO NUMERO VALE					
					IF upper(xnome_obj) =='TX_CODIGO_BARRA' 	
						...
						Else	
							...
							o_300027.lx_form1.lx_PAGEFRAME1.page2.LX_grid_filha1.Col_tx_CODIGO_BARRA.Tx_CODIGO_BARRA.value=curValeOld.codigo_barra
...
**************************************************
*-- Class:        bt_Desc_Obs (c:\work\desenv\120007\bt_Desc_Obs.vcx)
*-- ParentClass:  commandbutton
*-- BaseClass:    commandbutton
*-- Time Stamp:   10/10/08 04:41:11 PM
*
DEFINE CLASS bt_Desc_Obs AS commandbutton
	...
	PROCEDURE Click
		sele v_loja_venda_02
*--		retirado por wallace pacheco - 21/08/2015 : f_select("select anm_loja_obs from loja_venda where codigo_filial = ?V_LOJA_VENDA_02.CODIGO_FILIAL and ticket = ?V_LOJA_VENDA_02.TICKET and data_venda = ?V_LOJA_VENDA_02.DATA_VENDA","xDesc_Obs")
*--		Código criado abaixo por Wallace Pacheco - 21/08/2015
		f_select("select Motivo_desc from W_ANM_WRP_LOJA_EVENTOS_LOG_Motivo_desconto where codigo_filial = ?V_LOJA_VENDA_02.CODIGO_FILIAL and ticket = ?V_LOJA_VENDA_02.TICKET and terminal = ?V_LOJA_VENDA_02.TERMINAL and codigo_cliente = ?V_LOJA_VENDA_02.CODIGO_CLIENTE","xDesc_Obs")
		MESSAGEBOX(IIF(EMPTY(xDesc_obs.Motivo_desc),"Sem Observação",xDesc_obs.Motivo_desc))
	ENDPROC


ENDDEFINE
*
*-- EndDefine: bt_Desc_Obs 
**************************************************